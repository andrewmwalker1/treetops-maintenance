# [Your Business Name] Maintenance Platform — System Specification

*This is an adapted, de-identified copy of the specification for a
maintenance + H&S job-tracking platform originally built for a different,
unrelated business. It's included alongside `BUILD-BRIEF.md` as a
complete, accurate reference for replicating that working system exactly
— on this stack or another — in your own, entirely separate repo,
Supabase project, and deployment. Names, real staff, secrets, and the
original domain have been stripped out and replaced with placeholders;
everything else (architecture, data model, access-control rules, feature
behaviour) describes real, working software.*

*Reverse-engineered from the working application (33 SQL migrations, 7
Edge Functions, full React frontend + kiosk). This document describes
what is actually built and running in the original system, not just an
initial plan — treat it as the authoritative reference for a developer
(or an AI coding agent, working from these two files) tasked with
replicating this system, not `BUILD-BRIEF.md` alone.*

**Read the callout at the top of `BUILD-BRIEF.md` before starting** — in
short: the data model below supports multiple sites per organisation, but
no admin UI to create, switch, or manage additional sites was ever built.
Treat this as single-site software unless you plan to build that UI
yourself.
...
A maintenance and health & safety job-tracking Progressive Web App. The
original deployment was for a caravan park's gardening/maintenance team —
staff (gardeners, maintenance, office, management) and occasional
contractors tracked work items ("jobs") against pitches or free-text areas
of the park, attached photos and checklists, followed recurring
schedules, and managed a fleet of shared equipment (strimmers, mowers,
etc.) including fault reporting and a physical workshop check-out/check-in
kiosk driven by RFID fobs. None of that is specific to caravan parks —
"pitches" is just what that business called its locations, "gardeners" is
just what its operational staff were called, and the RFID kiosk is a
generic shared-tool-fleet check-out system. Rename freely for your own
business; the underlying mechanics below transfer directly.
...
organisation ([Your Business Name]) and one site exist in practice. The data model
...
scripts/
  seed-users.mjs                -- one-shot: invites the initial [Your Business Name] team
src/
...
|---|---|---|
| **organisations** | `id`, `name`, `created_at` | Root tenant. One row today ("[Your Business Name] Ltd"). |
| **sites** | `id`, `org_id→organisations`, `name`, `site_type` (text, default `'caravan_park'`), `terminology_overrides` (jsonb, nullable), `branding_overrides` (jsonb, nullable), `created_at` | One row today ("[Your Business Name]"). `terminology_overrides` and `branding_overrides` let a site override the org-level industry defaults per-key without a schema change. |
...
[Your Business Name] seed grants: **Admin** holds every permission. **Head Gardener**
...
§17 — acceptable while [Your Business Name] is the only tenant, must be wired up
...
the escape attempt it existed to stop (reproduced live during hands-on
testing of the workshop kiosk). The actual fix: `session.user.app_metadata.login_context`
...
that legitimately has desktop access (the Admin role, for instance)
keeps it, so this alone wouldn't have stopped the kiosk-escape described above.
...
| Contractors | `can_manage_contractors` | `contractors`: name, address, main email, main phone, notes, **`is_trusted`** checkbox (37-key-checkouts-and-contractor-reasons.sql — marks the handful of regular contractors who use the key station unaccompanied; needs no code of its own, just a normal profile + a role holding `can_use_key_system` + a registered fob, same as any staff member). Delete warns that assigned jobs become unassigned. A "Documents" button per contractor opens `ContractorDocumentsModal` (as before). A **"Key reasons"** button opens `KeyReasonsModal` (generic — see Key Reasons by Role below): add/delete `contractor_reasons` rows (label + sort_order) — shown as quick-pick buttons on the key-station check-out screen once this contractor is selected, always alongside a free-text override. |
| Key Reasons by Role | `can_manage_keys` | Lists every role with a "Reasons" button, each opening the same `KeyReasonsModal.jsx` the Contractors tab uses (originally `ContractorReasonsModal.jsx`, generalized to take `table`/`ownerColumn`/`ownerId` props once a second, near-identical use appeared, rather than keeping two copies) against `role_key_reasons` (39-role-key-reasons.sql) instead of `contractor_reasons`. These are presets for checking a key out to **yourself** — shown on the key-station check-out screen when "Me" is picked, keyed to the signed-in profile's own `role_id`. In the original deployment these were seeded from real examples per role/contractor — e.g. a caravan-prep role's presets were "Clean the caravan" / "At the request of the owner" / "Dress the caravan"; a heating contractor's were "Gas check" / "Carry out a repair" — reseed with whatever reasons actually recur for your own roles/contractors. |
...
No session yet. Full-screen "[Your Business Name] Workshop" scan prompt; any click
...
keys checked out right now." — so a contractor or member of staff can see
at a glance what they still have out before leaving site.
...
- **Check in (`/keys/checkin`)** — `KeySelector` over tags with an open
  ...
  for most people — deliberately, since keys in this business aren't
  always returned by whoever took them — **except** a contractor
  ...
  dedicated permission, both dropped once it became clear the existing
...
- **Find a key (`/keys/find`)** — `KeySelector` over every allocated tag;
  ...
  in the cupboard" / "No activity recorded yet"). This is the quick,
  non-admin answer to "where's this key" — the full history (every
...
- **Relocate (`/keys/relocate`) and Force check-in (`/keys/force-checkin`)**
  ...
  people (in the original deployment: one specific individual, via a role
  built for them alone, so granting `can_manage_keys` there doesn't reach
  anyone else — see
...
- **Hosting**: static build deployed to **GitHub Pages** via GitHub Actions (`.github/workflows/deploy.yml`), triggered on push to `main` or manual dispatch. Build env vars (`VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `VITE_VAPID_PUBLIC_KEY`) come from GitHub Secrets. Custom domain: `your-domain.example` (`public/CNAME`).
...
In the original deployment, a single seed SQL file created the
organisation, one site, five roles, three groups, four job statuses,
terminology defaults, and a `role_visibility` matrix (Admin → every role;
one supervisor-level role → itself plus the two operational roles beneath
it, not the office role; every other role → itself only — flagged in that
project as a placeholder assumption pending explicit confirmation from the
business owner, since "own role only" isn't necessarily right for every
team). A second seed file loaded real location codes from an actual CSV
export of the business's own sites/pitches.

For your own deployment: write the equivalent seed SQL for your own
organisation, site, roles, groups, statuses, terminology, and
`role_visibility` matrix (see Section 9 of `BUILD-BRIEF.md` for the shape
to follow) — don't reuse the original names, and decide `role_visibility`
deliberately rather than defaulting to "own role only."

A one-shot manual bootstrap script (run once, locally, with the service
role key) invites the initial team via `auth.admin.inviteUserByEmail` and
creates their `profiles`/`group_members`/`site_scope` rows. Write this
against your own real user list and real email addresses from the start —
the original checked-in script had exactly one real address (the
business owner's) and four placeholder ones for staff not yet onboarded,
explicitly skipped with a console warning rather than invited; that's a
pattern worth avoiding rather than replicating.
...
1. **No site picker.** The data model supports a user scoped to multiple sites, but the UI only ever auto-selects when exactly one site is in scope; with 0 or 2+, pages needing `activeSite` simply hang. Fine while [Your Business Name] is single-site; must be built before a second site is onboarded anywhere.
