...
// Andy: reference documents (an invoice, a manual) and date-critical
// certificates (Gas Test on staff room boilers, MOT on vehicles) per
// equipment item, each expiring (or not) independently -- one row per
// document rather than one blob per item, mirroring contractor
// documents (ContractorDocumentsModal.jsx). equipment-document-reminders
// (Edge Function) reads expiry_date + reminder_triggered_at from the
// same equipment_documents rows this modal writes.
export default function EquipmentDocumentsModal({ equipment, orgId, onClose }) {
  ...
  useEffect(refresh, [equipment.id]);
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      const path = `${equipment.id}/${docId}-${file.name}`;
      const { error: uploadError } = await supabase.storage.from("equipment-documents").upload(path, file, { upsert: true });
      if (uploadError) throw uploadError;

      const { error: pathError } = await supabase.from("equipment_documents").update({ storage_path: path }).eq("id", docId);
  ...
  async function handleDelete(doc) {
    ...
    if (doc.storage_path) await supabase.storage.from("equipment-documents").remove([doc.storage_path]);
    const { error: err } = await supabase.from("equipment_documents").delete().eq("id", doc.id);
    if (err) setError(err.message);
    else refresh();
  }

  return (
    <Modal title={`${equipment.name} — Documents`} onClose={onClose} maxWidth="520px">
        ...
        <form onSubmit={handleAdd}>
          ...
          <Input required value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g. MOT certificate, Gas Test cert" style={{ marginBottom: "var(--space-3)" }} />
          ...
          <p style={{ fontSize: "var(--text-xs)", color: colors.inkSoft, marginTop: "calc(-1 * var(--space-2))" }}>
            7 days before this date, a job is raised for the Office group to get it renewed.
          </p>

          <label className="tt-field__label">File (JPG or PDF)</label>
          <input required type="file" accept="image/jpeg,image/jpg,application/pdf" onChange={(e) => setFile(e.target.files?.[0] || null)} style={{ marginBottom: "var(--space-3)" }} />
