...
| **job_types** | `id`, `org_id`, `name`, `template_schema` (jsonb — ordered array of `{label, requiresPhoto}` items, optionally interleaved with `{type: "heading", label}` rows that name the section for the items after them), `requires_completion_photo` (bool, default false) | "Job templates" in the UI. Picking one on New Job pre-fills description/checklist/default activity types; the checklist remains freely editable afterward. `template_schema` was originally a plain array of label strings — migration 32 reshaped every existing row to `{label, requiresPhoto: false}` in place so per-item photo requirements (§6.3a) could be carried by a template. |
...
- **Checklist section** — shown if there are subtasks or the caller holds `can_edit_job_checklist`. Completed items get a strikethrough. Items with a `section` are grouped under collapsible section headings (all closed by default); unsectioned items list first.
  - **View mode is the default for everyone**, including editors — the same read-only list someone without `can_edit_job_checklist` sees, so ticking items off on a phone isn't crowded by editor controls. Checking items off and the per-item photo controls work in view mode.
  - **Edit mode** — an "Edit checklist" button at the bottom (only with `can_edit_job_checklist`) switches to it; "Done editing" switches back. Edit mode opens every section and adds inline rename, ↑/↓ reorder, remove, a per-item section picker, an "Add an item…" field (with a section picker and its own `can_require_checklist_item_photo`-gated "📷 Requires photo" checkbox), "Recall checklist…", and the same save/update-template buttons as New Job (gated on `can_edit_job_checklist && can_manage_reference_data`).
  - **Section headings in edit mode** — "Add section heading" creates a new heading (and preselects it in the add-item section picker); each heading's name is an inline-editable field (renaming updates `section` on every item in it) with a remove button (items stay, moving back to unsectioned; confirm-guarded if the section has items). A job's heading exists only as the `section` value on its items, so a new heading is held client-side until an item is put in it — leaving edit mode with it still empty discards it.
  - **Save/update template** serialises the checklist into `template_schema` *with* its headings: unsectioned items first, then per section a `{type: "heading", label}` row followed by its items — the same shape `ChecklistBuilder`, New Job and "Recall checklist" read back. (Before this, only `{label, requiresPhoto}` was saved, so a template made from a sectioned job came out flat.)

  Each row renders one of three ways (§6.3a):
...
- **Build-time metadata**: `vite.config.js` injects `__APP_VERSION__` (from `package.json`) and `__BUILD_TIME__`; `src/lib/buildInfo.js` turns them into one label, e.g. `v1.1.0 · 24 Sep 2026, 16:32` (build time in the device's timezone), shown in the desktop footer, the phone account menu and the kiosk corner. The git hash was dropped from it (2026-09-24) as meaningless to its readers. **Versioning** starts at 1.1.0 (2026-09-24): every deploy bumps the patch number (1.1.0 → 1.1.1); a minor bump (1.2.0) only when Andy calls a release — see CLAUDE.md.
